<x-app-layout>
    ...
    <form method="POST" action="{{ action([App\Http\Controllers\BookController::class, 'filter']) }}">
        ...
        <!-- Title -->
        <div>
            <x-label for="title" value="{{ __('messages.Title or portion of it') }}" />
        ...
        <!-- Year from -->
        <div>
            <x-label for="year_from" value="{{ __('messages.Year from') }}" />
        ...
        <!-- Year till -->
        <div>
            <x-label for="year_until" value="{{ __('messages.Year until') }}" />
        ...
        <!-- Abstract -->
        <div>
            <x-label for="abstract" value="{{ __('messages.Abstract or portion of it') }}" />
        ...
        <div>
            ...
            <x-multi-select id="author" class="block mt-1 w-full" name="author[]" :list='$authors' :selected="old('author')" text="{{ __('messages.Any author') }}"/>
        ...
        <div>
            ...
            <x-multi-select id="genre" class="block mt-1 w-full" name="genre[]" :list='$genres' :selected="old('genre')" text="{{ __('messages.Any genre') }}"/>
